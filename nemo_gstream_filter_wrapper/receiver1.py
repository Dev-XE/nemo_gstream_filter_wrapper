#!/usr/bin/env python3
"""
receiver-1.py  —  TAC 2026  |  Camera 1  Logitech C270  (port 5001)
=====================================================================
CAMERA FIXES APPLIED (all 8 problems, see underwater_enhance.py):
  1. Curved acrylic blur        -> deblur kernel + unsharp mask
  2. Blue/green color cast      -> gray-world white balance + red boost
  3. Low contrast / haze        -> CLAHE on luminance (LAB)
  4. Backscatter particles      -> median filter
  5. Low brightness underwater  -> gamma correction
  6. C270 barrel distortion     -> undistort remap (C270 coefficients, stronger)
...
KEYS (runtime):
  q   quit
  t   toggle TAC mode (turbid harbour — dehaze + aggressive CLAHE)

USAGE:
  ros2 run nemo_logitech_wrapper receiver_1   (default port 5001)
  ros2 run nemo_logitech_wrapper receiver_1 --ros-args -p port:=5001
"""
...
# ── shared enhancement module (same package directory)
from underwater_enhance import enhance, enhance_for_aruco, CAMERA_MATRICES, DIST_COEFFS

CAM_ID = 1   # Logitech C270 — matches calibration profile in underwater_enhance.py
...
# ── FPS MODE ──────────────────────────────────
# --jetson  → full enhance pipeline (Jetson over ethernet, targets 40 FPS)
# (none)    → lightweight mode for laptop-only, targets 30+ FPS
import sys as _sys
JETSON_MODE  = "--jetson" in _sys.argv
ENHANCE_SKIP = 1 if JETSON_MODE else 2   # enhance every frame (jetson) or every 2nd (laptop)
...
_tac_flag = [False]   # runtime toggle via 'T' key
...
def handle_docking_event(aruco_id, tx, ty, now):
    print(f"[DOCKING] Tag ID: {aruco_id} | X:{tx} Y:{ty}")
...
def contour_score_for_pipe(c):
    ...
    if short_side <= 1 or (long_side / short_side) < PIPE_MIN_ASPECT or long_side < PIPE_MIN_LENGTH:
        return -1, None
    if (area / (rw * rh)) < PIPE_MIN_FILL_RATIO: return -1, None
    hull_area = cv2.contourArea(cv2.convexHull(c))
    if hull_area <= 1 or (area / hull_area) < PIPE_MIN_SOLIDITY: return -1, None
    return (area * 0.002 + (long_side / short_side) * 120 +
            (area / (rw * rh)) * 200 + (area / hull_area) * 150 + long_side * 0.8), rect
...
def make_aruco_worker(det_idx):
    aruco_dict = aruco_dicts[det_idx]
    ...
    def worker():
        while True:
            try:
                enh_gray_s, scale = in_q.get(timeout=1.0)
            except queue.Empty:
                continue

            corners, ids, _ = aruco.detectMarkers(enh_gray_s, aruco_dict, parameters=aruco_params)
            ...
            for i in range(len(ids)):
                ...
                try:
                    aruco_result_queue.put_nowait(({
                        ...
                        "aruco_id": aruco_id,
                        "role":     role,
                        "corner":   (corners[i] / scale).astype(np.float32),
                        "aid":      ids[i].copy(),
                    }, ts))
                except queue.Full:
                    pass
...
def draw_enhance_badge(frame, tac: bool, brightness: float, mode: str, w: int):
    badge_x = w - 195
    items = [
        (f"CAM: C270 (cam{CAM_ID})", C_DIM),
        (f"MODE: {mode}",            C_OK if mode == "DAY" else C_WARN),
        (f"BRIGHT: {int(brightness)}", C_DIM),
        (f"TAC: {'ON [T]' if tac else 'OFF [T]'}", C_TAC if tac else C_DIM),
        ("WB+RED  CLAHE  GAMMA",      C_DIM),
        ("DEBLUR  SHARP  UNDIST",     C_DIM),
    ]
    for i, (txt, col) in enumerate(items):
        cv2.putText(frame, txt, (badge_x, 20 + i * 16),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.38, col, 1, cv2.LINE_AA)


# ─────────────────────────────────────────────
#  ROS 2 NODE  (port 5001 — C270 stream)
# ─────────────────────────────────────────────
class GstRxNode(Node):
    def __init__(self):
        super().__init__("gst_rx_node_cam1")
        self.declare_parameter("port",       5001)   # C270 uses port 5001
    ...
    def get_pipeline_string(self) -> str:
        ...
        sync_str = "true" if bool(self.get_parameter("sync").value) else "false"
        pipeline = (
            f'udpsrc port={port} '
        ...
        self.get_logger().info(f"[Cam{CAM_ID}] GStreamer pipeline:\n{pipeline}")
        return pipeline
...
def detection_main(cap):
    ...
    print(f"[INFO] Cam{CAM_ID} C270  Pipeline+ArUco — press Q to quit, T to toggle TAC mode")
    print(f"[INFO] Mode: {'JETSON ETHERNET (full enhance, 40 FPS target)' if JETSON_MODE else 'LAPTOP ONLY (lightweight, 30+ FPS target)'}")
    ...
    while True:
        ...
        # ── Apply enhance every Nth frame to save CPU in laptop mode
        if _frame_count % ENHANCE_SKIP == 0 or _last_enhanced is None:
            frame = enhance(raw_frame, CAM_ID, tac_mode=tac)
            _last_enhanced = frame
        else:
            frame = _last_enhanced   # reuse last enhanced frame

        h, w   = frame.shape[:2]
        cx, cy = w // 2, h // 2
        now    = time.monotonic()
        ct     = time.time()
        fps    = 1.0 / max(ct - prev_time, 1e-6)
        prev_time = ct

        # ── Pipeline mask on corrected frame
        ...
        # ── ArUco detection on strongly enhanced grayscale
        enh_gray   = enhance_for_aruco(raw_frame, CAM_ID, tac_mode=tac)
        brightness = float(cv2.mean(enh_gray)[0])
        mode       = "NIGHT" if brightness < NIGHT_THRESHOLD else "DAY"
        enh_gray_s = cv2.resize(enh_gray, (0, 0), fx=DETECT_SCALE, fy=DETECT_SCALE)

        for q in aruco_input_queues:
            force_put(q, (enh_gray_s, DETECT_SCALE))
        ...
        if fresh_aruco:
            ...
            for i in range(len(all_det)):
                ...
                for j in range(i + 1, len(all_det)):
                    if not used[j] and (
                        (all_det[i][2]-all_det[j][2])**2 +
                        (all_det[i][3]-all_det[j][3])**2
                    )**0.5 < SPATIAL_MERGE_DIST:
            ...
            for group in groups:
                for idx in group:
                    if (all_det[idx][0], all_det[idx][1]) > best_score:
                        best_score = (all_det[idx][0], all_det[idx][1])
            ...
            if best_r:
                ...
                if role == "PIPELINE":
                    handle_pipeline_event(aid, now)
                elif role == "DOCKING" and aid != prev_detected_id:
                    handle_docking_event(aid, best_r["tx"], best_r["ty"], now)
        ...
        if detected is not None:
            ...
            if lost_frames >= MAX_LOST:
                last_target, last_marker_result, prev_detected_id = None, None, None
        ...
        title = "CAM1 C270  |  PIPELINE INSPECTION"
        if tac:
            title += "  [TAC]"
        cv2.putText(frame, title, (14, 32),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, C_TAC if tac else C_TITLE, 1, cv2.LINE_AA)
        cv2.putText(frame, f"FPS: {int(fps)}", (w - 80, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.44, C_OK, 1, cv2.LINE_AA)

        draw_enhance_badge(frame, tac, brightness, mode, w)

        cv2.putText(frame, "TELEMETRY", (18, PANEL_LEFT_Y1 + 22),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.52, C_TITLE, 1, cv2.LINE_AA)
        ...
        if last_target is not None and lost_frames < MAX_LOST:
            ...
            cv2.putText(frame, "CENTER LOCKED" if centered else "ALIGNING",
                        (18, ROW_START + ROW_GAP*6 + 6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, C_OK if centered else C_BAD, 1, cv2.LINE_AA)
            ...
            cv2.putText(frame, f"ID {aid}", (tx + 14, ty - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.44, C_ACCENT, 1, cv2.LINE_AA)
        else:
            for i, (l, v) in enumerate([
                ("TARGET ID", "--"), ("ROLE", "NONE"), ("DX", "--"), ("DY", "--"),
                ("PINGER", mission.pinger_side.upper()),
                ("PIPE ANG", f"{mission.pipeline_angle:.1f}d"),
            ]):
                draw_row(frame, l, v, ROW_START + ROW_GAP * i, C_BAD if i < 2 else C_DIM)
            cv2.putText(frame, "NO TARGET", (18, ROW_START + ROW_GAP*6 + 6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, C_BAD, 1, cv2.LINE_AA)

        draw_crosshair(frame, cx, cy)
        if pipe_found:
            cv2.circle(frame, (pipe_cx, pipe_cy), 5, C_PIPE, -1)
            cv2.putText(frame, "PIPE", (pipe_cx + 8, pipe_cy - 8),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.40, C_PIPE, 1, cv2.LINE_AA)

        cv2.putText(frame,
                    f"STATUS: {'PIPELINE DETECTED' if mission.pipeline_found else 'SEARCHING PIPELINE'}",
                    (14, h - 14), cv2.FONT_HERSHEY_SIMPLEX, 0.52,
                    C_OK if mission.pipeline_found else C_BAD, 1, cv2.LINE_AA)
        cv2.putText(frame, f"MARKERS: {mission.marker_count}",
                    (w - 160, h - 14), cv2.FONT_HERSHEY_SIMPLEX, 0.48, C_DIM, 1, cv2.LINE_AA)

        draw_toast(frame, now, w, h)

        cv2.imshow("Cam1 C270  |  Pipeline Inspection", frame)
        cv2.imshow("Cam1 Pipeline Mask", mask_vis)

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
        elif key == ord('t'):
            _tac_flag[0] = not _tac_flag[0]
            state = "ON" if _tac_flag[0] else "OFF"
            print(f"[Cam{CAM_ID}] TAC (turbid harbour) mode: {state}")
            toast.show(f"TAC MODE {state}", "dehaze + aggressive CLAHE")
...
def main():
    ...
    if not cap.isOpened():
        node.get_logger().error(
            "Failed to open UDP stream (port 5001). "
            "Check that transmitter-1.py is running on the Jetson."
        )
        return
