#!/usr/bin/env python3
"""
receiver.py  —  TAC 2026  |  Camera 0  Groov-e  (port 5000)
=============================================================
CAMERA FIXES APPLIED (all 8 problems, see underwater_enhance.py):
  ...
  6. Barrel distortion          -> undistort remap (Groov-e coefficients)
...
USAGE:
  ros2 run nemo_logitech_wrapper receiver   (default port 5000)
  ros2 run nemo_logitech_wrapper receiver --ros-args -p port:=5000
"""
...
# ── shared enhancement module (drop in same package directory)
from .underwater_enhance import enhance, enhance_for_aruco, CAMERA_MATRICES, DIST_COEFFS

CAM_ID = 0   # Groov-e camera — matches calibration profile in underwater_enhance.py
...
C_PANEL_BG = ( 18,  18,  18)
C_TITLE    = (160, 140,  90)
C_TAC      = (  0, 200, 255)   # TAC-mode indicator colour
...
# Runtime TAC-mode toggle (shared with ArUco worker threads via list reference)
_tac_flag = [False]
...
def compute_pipeline_mask(frame):
    """Run pipeline colour segmentation on the already-enhanced frame."""
...
#  Each worker receives an enhance_for_aruco()-processed grayscale
#  frame so all 8 fixes are applied before detection.
# ─────────────────────────────────────────────
...
def draw_enhance_badge(frame, tac: bool, brightness: float, mode: str, w: int):
    """Small overlay showing active enhancement status."""
    badge_x = w - 195
    badge_items = [
        (f"CAM: GROOV-E (cam{CAM_ID})", C_DIM),
    ...
    for i, (txt, col) in enumerate(badge_items):
...
class GstRxNode(Node):
    def __init__(self):
        super().__init__("gst_rx_node_cam0")
...
def detection_main(cap):
    ...
    print(f"[INFO] Cam{CAM_ID} Groov-e  Pipeline+ArUco — press Q to quit, T to toggle TAC mode")
    ...
    while True:
        ...
        # ── Apply enhancement to every frame (removes flickering from frame skip)
        ...
        # ── Pipeline colour detection (runs on enhanced frame — colours are correct)
        ...
        # ── Brightness-adaptive ArUco prep
        #    enhance_for_aruco() applies undistort + WB + gamma + strong CLAHE + deblur
        ...
        title = "CAM0 GROOV-E  |  PIPELINE INSPECTION"
        if tac:
            title += "  [TAC]"
        ...
        cv2.imshow("Cam0 GROOV-E  |  Pipeline Inspection", frame)
        cv2.imshow("Cam0 Pipeline Mask", mask_vis)
...
def main():
    ...
    if not cap.isOpened():
        node.get_logger().error(
            "Failed to open UDP stream via GStreamer! "
